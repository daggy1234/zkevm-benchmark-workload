...
use reth_chainspec::{Chain, NamedChain, HOLESKY, HOODI, SEPOLIA, mainnet_chain_config};
...
fn classify(n_state: usize) -> &'static str {
    if n_state < 8_000 { "light" }
    else if n_state < 12_000 { "moderate" }
    else if n_state < 18_000 { "heavy" }
    else { "extreme" }
}
...
async fn main() -> Result<()> {
    ...
    if let SourceCommand::Rpc { ref rpc_url, ref rpc_header, last_n_blocks, block, follow } = cli.source {
        if !follow {
...
async fn run_rpc_with_timing(
    ...
    let chain_id = EthApiClient::<(), (), (), (), (), ()>::chain_id(&client)
        .await?.ok_or_else(|| anyhow!("no chain_id"))?;
    ...
    let latest = EthApiClient::<TransactionRequest, Transaction, Block, Receipt, Header, TransactionSigned>::block_by_number(
        &client, BlockNumberOrTag::Latest, false,
    ).await?.ok_or_else(|| anyhow!("no latest block"))?;
    ...
    info!("Processing {} blocks ({} → {})", blocks.len(),
        blocks.last().unwrap_or(&0), blocks.first().unwrap_or(&0));
        ...
        if !no_save {
            ...
            if fixture_path.exists() {
                ...
                if skipped <= 5 || skipped % 50 == 0 {
                    info!("[{}/{}] block {bn}: skipping (fixture exists)", i + 1, blocks.len());
                }
                continue;
            }
        }
    ...
    info!("Done: {ok} ok, {errors} errors, {skipped} skipped in {:.1}s", total_start.elapsed().as_secs_f64());
...
async fn process_one_block(
    ...
    let block = EthApiClient::<
        TransactionRequest, Transaction, Block<TransactionSigned>, Receipt, Header, TransactionSigned,
    ...
    let gas_pct = if gas_limit > 0 { gas_used as f64 / gas_limit as f64 * 100.0 } else { 0.0 };

    // Time the witness generation RPC call
    let t_witness = Instant::now();
    let witness = DebugApiClient::<()>::debug_execution_witness(
        client, BlockNumberOrTag::Number(block_num),
    ).await?;
...
async fn build_generator(source: SourceCommand) -> Result<Box<dyn FixtureGenerator>> {
    match source {
        SourceCommand::Tests { tag, include, exclude, eest_fixtures_path } => {
            let mut builder = EESTFixtureGeneratorBuilder::default();
            if let Some(tag) = tag { builder = builder.with_tag(tag); }
            else if let Some(p) = eest_fixtures_path { builder = builder.with_input_folder(p)?; }
            if let Some(inc) = include { builder = builder.with_includes(inc); }
            if let Some(exc) = exclude { builder = builder.with_excludes(exc); }
        ...
        SourceCommand::Rpc { rpc_url, rpc_header, follow, last_n_blocks, block } => {
            ...
            } else if let Some(b) = block { builder = builder.block(b); }
            else { builder = builder.last_n_blocks(last_n_blocks.unwrap_or(1)); }
            Ok(Box::new(builder.build().await?))
        }
    }
}
